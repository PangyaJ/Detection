...
def train_model(model, dataloader, criterion, optimizer, num_epochs, device, save_path):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_avg_loss < best_loss:
            ...
            best_epoch = epoch + 1
            best_model_path = f"[EfficientNet_b7]{save_path}_best_epoch_{best_epoch}_Loss_{best_loss:.4f}.pth"
        ...
        if (epoch + 1) % 10 == 0:
            epoch_no = epoch + 1
            epoch_save_path = f"[EfficientNet_b7]{save_path}_epoch_{epoch_no}_Loss_{best_loss:.4f}.pth"
...
def main():
    ...
    loaded_model.load_state_dict(torch.load(f"[EfficientNet_b7]{save_path}_epoch_50_.pth"))
    loaded_model.eval()
