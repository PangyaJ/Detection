...
# EfficientNet 모델 정의
def get_densenet169(num_classes):
...
def train_model(model, dataloader, criterion, optimizer, num_epochs, device, save_path):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_avg_loss < best_loss:
            ...
            best_epoch = epoch + 1
            best_model_path = f"[densenet169_transfer]{save_path}_best_epoch_{best_epoch}_Loss_{best_loss:.4f}.pth"
        ...
        if (epoch + 1) % 10 == 0:
            epoch_no = epoch + 1
            epoch_save_path = f"[densenet169_transfer]{save_path}_epoch_{epoch_no}_Loss_{best_loss:.4f}.pth"
...
def main():
    ...
    #=============================================================================#
    # 학습된 모델 가중치 로드
    pretrained_path = ('C:/Users/Evan/OneDrive/바탕 화면/VScode/[1st]train/[DenseNet169]trained_densenet169_model_best_epoch_50_Loss_0.1602.pth')
    ...
    print("\n저장된 모델 확인:")
    loaded_model = get_densenet169(num_classes).to(device)
    loaded_model.load_state_dict(torch.load(f"[densenet169_transfer]{save_path}_epoch_50_.pth"))
    loaded_model.eval()
